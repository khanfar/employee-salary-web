    ...
    for row in reader:
        if row['تقرير نهائي'] and 'شيكل' in row['تقرير نهائي']:
            ...
            if start_date <= entry_date <= end_date:
                ...
                if not amount:
                    print(f"No valid amount found in row: {row}")
                    continue  # Skip rows with invalid amount format

                amount_value = float(amount[0][0])

                if mechanic in mechanics_work:
                    mechanics_work[mechanic]['job_count'] += 1
                    mechanics_work[mechanic]['total_money'] += amount_value
                else:
                    mechanics_work[mechanic] = {'job_count': 1, 'total_money': amount_value}
